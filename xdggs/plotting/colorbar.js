...
function render({ model, el }) {
  ...
  const svg = d3.select(el).append("svg").attr("width", width);
  ...
  // axis label
  const labelY = marginTop + rampHeight + axisHeight + labelSpacer;
  const label = svg
    .append("text")
  ...
  // final styling
  const finalHeight = labelY + labelHeight + marginBottom;
